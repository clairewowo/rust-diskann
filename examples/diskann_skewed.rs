...
fn main() -> Result<(), DiskAnnError> {
    ...
    // example of running it on a dataset with 48 max neighbors and alpha=1.2:
    // ./diskann_skewed input.hdf5 48 1.2
    ...
    if args.len() == 2 { // just the file output name provided, use default
        max_degree = 48;
    }
    else { // passed in max degree
        max_degree = args[2].parse().unwrap();
    }
